    ...
    for line in io.lines(env_file) do
      if not line:match("^%s*#") and line:match("=") then
        ...
        if key and value then
          ...
          -- Remove aspas se houver nas pontas
          value = value:gsub("^['\"]", ""):gsub("['\"]$", "")

          -- Injeta estritamente como string literal no ambiente do processo do Vim
          vim.env[key] = value
...
-- Inicializa a tabela global de bancos de forma limpa, extraindo as strings puras
vim.g.dbs = {
  vendas_mysql = vim.env.DB_MYSQL,
  locadora_oracle = vim.env.DB_ORACLE_TESTE,
}
